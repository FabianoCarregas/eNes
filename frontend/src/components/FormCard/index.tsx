...
type Props = {
  ...
  function FormCard({ gameId }: Props) {
    ...
    useEffect(() => {
      axios.get(`${BASE_URL}/games/${gameId}`).then((response) => {
    ...
    return (
      <div className="egames-form-container">
        <img
          className="egames-game-card-image"
          src={game?.image}
          alt={game?.title}
        />
        <div className="egames-card-bottom-container">
          ...
          <form className="egames-form" onSubmit={handleSubmit}>
            ...
            <div className="egames-form-btn-container">
              <button type="submit" className="btn btn-primary egames-btn">
                Salve
              </button>
            </div>
          </form>
          <Link to="/">
            <button className="btn btn-primary egames-btn mt-3">Cancel</button>
          </Link>
        </div>
      </div>
    );
